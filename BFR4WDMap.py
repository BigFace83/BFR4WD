...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        # Map Canvas
        ##################################################################
        self.Map = Canvas(self, width=600, height=600)
        self.Map.grid(row = 0, column = 0, columnspan = 3, rowspan = 3)
    ...
    def DrawMap(self,event):
        ...
        self.MapForImage = self.MapArray #copy MapArray and use the copy to modify for displaying
        for x in np.nditer(self.MapForImage , op_flags=['readwrite']): #Loop through entire array
            x[...] = 255-(x* 255) #Convert from a float array (0 to 1) to a grayscale image (0-255)
        

        self.MapForImage  = self.MapForImage .astype(np.uint8) #Make array and array of integers

        self.MapImage = cv2.cvtColor(self.MapForImage ,cv2.COLOR_GRAY2RGB) #Convert to RGB before adding coloured objects
